module deadcode.animation.mutator;
...
private string getProxyFields(T, alias prefix = "")()
{
	...
	foreach (_field; __traits(allMembers,T))
	{
		...
		//static if (_field == "this" || _field == "Monitor" || field[$-1] == '.' || !__traits(compiles, mixin("T." ~ _field)))
        static if (_field == "this" || _field == "Monitor" || field[$-1] == '.' || !IsAccessible!(T, _field))
		{
			continue;
		}
